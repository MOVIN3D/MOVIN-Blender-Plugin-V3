...
class MOVINRuntime:
    def __init__(self):
        ...
        self.bone_local_position_reference = {}
    ...
    def reset(self):
        with self.lock:
            ...
            self.bone_local_position_reference.clear()
            self.last_applied = None
            self.last_pointcloud_frame = None
            self.last_actor = ""
            self.last_ts = ""
...
class MOVIN_Props(PropertyGroup):
    ...
    apply_bone_local_position: BoolProperty(
        name="Apply Bone Local Position Delta",
        description="Apply streamed per-bone local position changes relative to the first received frame",
        default=False
    )
    bone_local_position_scale: FloatProperty(
        name="Bone Local Position Scale",
        description="Scale streamed per-bone local position deltas before applying them to pose bones",
        default=100.0
    )
...
def _apply_latest_stream_data(scene_name):
    ...
    if frame is not None and arm_obj is not None and arm_obj.type == 'ARMATURE':
        ...
        for name, bdat in by_name.items():
            ...
            if name == hips_bone_name:
            ...
            elif props.apply_bone_local_position:
                reference = _runtime.bone_local_position_reference.get(name)
                if reference is None:
                    _runtime.bone_local_position_reference[name] = p
                    reference = p

                bone_local_position_scale = props.bone_local_position_scale
                delta = (
                    p[0] - reference[0],
                    p[1] - reference[1],
                    p[2] - reference[2],
                ...
                pb.location = (
                    delta[0] * bone_local_position_scale,
                    delta[1] * bone_local_position_scale,
                    delta[2] * bone_local_position_scale,
                )
...
class MOVIN_OT_ResetBoneLocalPositionReference(Operator):
    bl_idname = "movin.reset_bone_local_position_reference"
    bl_label = "Reset Local Position Reference"
    bl_description = "Use the next received frame as the reference for bone local position deltas"
    def execute(self, context):
        global _runtime
        _runtime.bone_local_position_reference.clear()
        self.report({'INFO'}, "Bone local position reference will be reset on the next frame")
        return {'FINISHED'}
...
class MOVIN_PT_Panel(Panel):
    ...
    def draw(self, context):
        ...
        row = box.row(align=True)
        row.prop(props, "apply_bone_local_position")
        row = box.row(align=True)
        row.enabled = props.apply_bone_local_position
        row.prop(props, "bone_local_position_scale")
        row = box.row(align=True)
        row.enabled = props.apply_bone_local_position
        row.operator("movin.reset_bone_local_position_reference", text="Reset Local Position Reference", icon="FILE_REFRESH")
...
classes = (
    ...
    MOVIN_OT_ResetBoneLocalPositionReference,
    MOVIN_OT_Start,
    MOVIN_OT_Stop,
    MOVIN_OT_DumpStatus,
    MOVIN_PT_Panel,
